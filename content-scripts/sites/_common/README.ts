...
import ReadMeButton from "@/components/injected-ui/README.vue";
import { selectors } from "../selectors";
import { waitForElementByXPath, waitForElementBySelector, observeDOM } from "@/utils/dom-helpers";
import { logger } from '@/utils/logger';
import { ContentScriptContext } from "#imports";

// import css framework here because 
//! important: always add '?inline' at the end of the file name
    ...
    try {
        ...
        if (playBar && !playBar.querySelector('.michibiki-injected-button')) {
            ...
            const { unmount, componentName } = await mountInjectedComponent(
                context, // required
                ReadMeButton, // vue component
                mountPoint, // a parent div that the component will be injected in
                { msg: 'Ext Action'}, // component props
                'Component name here', // component name (anything you want)
                pureCssContent // inline CSS for the component, you can add multiple files like this: cssFile1 + cssFile2 + cssFile3 and so on...
            );
